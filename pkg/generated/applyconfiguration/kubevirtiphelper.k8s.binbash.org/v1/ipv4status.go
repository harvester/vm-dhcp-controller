...
// IPv4StatusApplyConfiguration represents an declarative configuration of the IPv4Status type for use
// with apply.
...
// IPv4StatusApplyConfiguration constructs an declarative configuration of the IPv4Status type for use with
// apply.
func IPv4Status() *IPv4StatusApplyConfiguration {
	return &IPv4StatusApplyConfiguration{}
}

// WithAllocated puts the entries into the Allocated field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, the entries provided by each call will be put on the Allocated field,
// overwriting an existing map entries in Allocated field with the same key.
func (b *IPv4StatusApplyConfiguration) WithAllocated(entries map[string]string) *IPv4StatusApplyConfiguration {
	if b.Allocated == nil && len(entries) > 0 {
		b.Allocated = make(map[string]string, len(entries))
	}
