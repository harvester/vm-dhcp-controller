...
type VirtualMachineNetworkConfigController interface {
	generic.ControllerInterface[*v1alpha1.VirtualMachineNetworkConfig, *v1alpha1.VirtualMachineNetworkConfigList]
}
...
type VirtualMachineNetworkConfigClient interface {
	generic.ClientInterface[*v1alpha1.VirtualMachineNetworkConfig, *v1alpha1.VirtualMachineNetworkConfigList]
}
...
type VirtualMachineNetworkConfigCache interface {
	generic.CacheInterface[*v1alpha1.VirtualMachineNetworkConfig]
}
...
// RegisterVirtualMachineNetworkConfigStatusHandler configures a VirtualMachineNetworkConfigController to execute a VirtualMachineNetworkConfigStatusHandler for every events observed.
...
// RegisterVirtualMachineNetworkConfigGeneratingHandler configures a VirtualMachineNetworkConfigController to execute a VirtualMachineNetworkConfigGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured VirtualMachineNetworkConfigGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *virtualMachineNetworkConfigGeneratingHandler) isNewResourceVersion(obj *v1alpha1.VirtualMachineNetworkConfig) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *virtualMachineNetworkConfigGeneratingHandler) storeResourceVersion(obj *v1alpha1.VirtualMachineNetworkConfig) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
